...
function numCell(
    ...
    editMode: boolean,
    draft: Record<number, Partial<RentalPricingUpdateItem>>,
    ...
    const draftValue = (draft[id]?.[field] as number | undefined) ?? value;
    return (
        <Input
            type="number"
            min={0}
...
export function createRentalPricingColumns(
    ...
    return [
        ...
        helper.accessor('rate1Day', {
            header: '1-Day Rate',
            cell: (info) => numCell(info.getValue(), info.row.original.id, 'rate1Day', editMode, draft, onUpdate),
        }),
        helper.accessor('rate3Days', {
            header: '3-Day Rate',
            cell: (info) => numCell(info.getValue(), info.row.original.id, 'rate3Days', editMode, draft, onUpdate),
        }),
        helper.accessor('ratePerDayExtra', {
            header: 'Extra Day Rate',
            cell: (info) => numCell(info.getValue(), info.row.original.id, 'ratePerDayExtra', editMode, draft, onUpdate),
        }),
        helper.accessor('latePenaltyPerDay', {
            header: 'Late Penalty / Day',
            cell: (info) => numCell(info.getValue(), info.row.original.id, 'latePenaltyPerDay', editMode, draft, onUpdate),
        }),
    ];
}
